...
str=setstr(real('\')*[1+0*x(:,1)]);
str=[str str];


%work backwards
andv=setstr(real('&')*[1+0*x(:,1)]);
blankv=setstr(real(' ')*[1+0*x(:,1)]);
for i=size(x,2):-1:1
	if i==size(x,2)
	     	str=[vint2str(x(:,i),prec(i),'spaces') blankv str];
	else
		str=[vint2str(x(:,i),prec(i),'spaces') blankv andv blankv str];
	end
end
