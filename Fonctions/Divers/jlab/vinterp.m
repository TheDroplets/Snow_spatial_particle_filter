function[yi]=vinterp(x,y,xi,str);
%VINTERP Matrix-matrix 1-D interpolation.
%
%   YI=VINTERP(X,Y,XI), returns the linear interpolation of Y onto XI     
%   based on the functional relationship Y(X).                            
%                                                                         
%   Unlike INTERP1, VINTERP allows X,Y, and XI to be either vectors or    
...
for i=1:size(x,2)
 	colmin=min(x(find(isfinite(x(:,i))),i));
	colmax=max(x(find(isfinite(x(:,i))),i));
