...
for m = 1 :length(MASQUE)
    ...
    p = patch(MASQUE(m).X(ind),MASQUE(m).Y(ind),999*ones(size(MASQUE(m).Y(ind))),[1 1 1],'tag','masque');
...
for l = 1 : length(LACS);
    if LACS(l).SUPERFICIE >= plot_info.sup_lacs_min
        ...
        p = patch(LACS(l).X(ind),LACS(l).Y(ind),-999*ones(size(LACS(l).Y(ind))),[1 1 1],'tag','lacs');
          set(get(get(p,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');      
...
for i = 1 : length(ILES)
    ...
    if ILES(i).SUPERFICIE >= plot_info.sup_iles_min
        ...
        try
            p = patch(ILES(i).X(ind),ILES(i).Y(ind),999*ones(size(ILES(i).Y(ind))),[1 1 1],'tag','iles');
            set(get(get(p,'Annotation'),'LegendInformation'),'IconDisplayStyle','off');
        catch
            disp('bug');
        end
